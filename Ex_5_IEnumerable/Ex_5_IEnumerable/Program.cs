    ...
    public class Ellipse : IFigure
    {
        ...
        public double Perimeter()
        {
            return radius_1 == radius_2
                ? 2 * Math.PI * radius_1
                : 4 * ((Math.PI * radius_1 * radius_2 + Math.Pow(radius_1 - radius_2, 2)) / (radius_1 + radius_2));
        }

        public double Area()
        {
            return Math.PI * radius_1 * radius_2;
        }

        public string SpecialProperties()
        {
            return radius_1 == radius_2 ? "Круг" : "Эллипс";
        }

        public void Info()
        {
            Console.WriteLine($"{SpecialProperties(),16}\t({radius_1},{radius_2})\t \t{Perimeter(),10:f2}\t{Area(),10:f2}");
        }
    }
